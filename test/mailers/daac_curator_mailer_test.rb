...
class DaacCuratorMailerTest < ActionMailer::TestCase
  ...
  context 'released_records_digest_notification email appearance' do
    should 'correctly populates the fields of an email' do
  ...
  context 'finding all the users and records to send digest e-mails' do
    context 'when two different daacs need e-mails' do
      ...
      should 'sends two e-mails when two DAACs have records in review' do
    ...
    context 'when users with different preferences are processed' do
      ...
      should 'sends one email to one recipient on a Tuesday' do
      ...
      should 'sends one email to four recipients on the first Monday' do
      ...
      should 'sends one email to two recipients on the second Monday' do
        DaacCuratorMailerOrchestration.released_records_digest_conductor(now: Time.new(2020, 2, 10))
      ...
      should 'sends one email to three recipients on the third Monday' do

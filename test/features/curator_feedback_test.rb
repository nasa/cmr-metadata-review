...
class CuratorFeedbackTest < SystemTestCase
  ...
  test 'verifies no records in Requires Reviewer Feedback Records section.' do
  ...
  test 'only daac curators can click the curator feedback button' do
  ...
  test 'Curator Feedback with a field requesting feedback' do
      mock_login(id: 5) # daac curator
      visit '/home'
      within '#in_daac_review' do
        all('#record_id_')[0].click
  ...
  test 'verifies the record shows up in Requires Reviewer Feedback Records section for the daac curator.' do
    mock_login(id: 5) # daac curator
    visit '/home'
    within '#in_daac_review' do
      all('#record_id_')[0].click
  ...
  test 'curator feedback shows up for arc curator' do
    mock_login(id: 5) # daac curator
    visit '/home'
    within '#in_daac_review' do
      all('#record_id_')[0].click

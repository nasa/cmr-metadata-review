...
const PaginationItemView = observer(
  class PaginationItemView extends React.Component {
    ...
    selectPage(pageToSelect) {
      disableAllButtons(this.props.formId);
      this.props.viewModel.selectPage(pageToSelect)
    }
    ...
    render() {
      ...
      if (name === 'First' || name === 'Last') {
        ...
        return (
          <li className="paginationItem" style={style}><a onClick={() => {
      ...
      let lineItem = <li key={value} className="paginationItem" style={style}><a onClick={() => {
        this.selectPage(value)
      }}> {this.props.children}</a></li>
      if (this.props.active) {
        style.backgroundColor = "#2c3e50"
        lineItem = <li key={value} className="paginationItem" style={style}><a onClick={() => {
          this.selectPage(value)
        }} className="active-page">{this.props.children}</a></li>
      }
      return (
        <React.Fragment>
          {lineItem}

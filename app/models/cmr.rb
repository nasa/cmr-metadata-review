class Cmr
  ...
  def self.format_collection(raw_collection, data_format = "echo10")
    desired_fields = if data_format == "echo10"
      CombinedDesiredFields.get_format_fields('echo10')
    elsif data_format == "dif10"
      CombinedDesiredFields.get_format_fields('dif10')
    elsif data_format == "umm_json"
      CombinedDesiredFields.get_format_fields('ummc')
    else
      []
    end
  ...
  def self.format_granule_data(granule_raw_data)
    ...
    add_required_fields(results_hash, CombinedDesiredFields.get_format_fields('echo10_granule'))
  end
